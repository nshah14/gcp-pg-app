...
public class RevUtils {
	...
	public boolean isValidBirthDate(String date) {
		...
		System.out.println("year " + Integer.parseInt(bDate[0]) + "  " + today.getYear() + "status "
				+ (Integer.parseInt(bDate[0]) > today.getYear()));
		System.out.println("month " + Integer.parseInt(bDate[1]) + " " + today.getMonthValue() + "status "
				+ (Integer.parseInt(bDate[1]) > today.getMonthValue()));
		System.out.println("date " + Integer.parseInt(bDate[2]) + " " + today.getDayOfMonth() + "status "
				+ (Integer.parseInt(bDate[2]) > today.getDayOfMonth()));
	...
	private long getNumberOfDaysToBday(String strBDay) {
		...
		LOGGER.info("OUT:: getNumberOfDaysToBday  month to birthday  :: " + period.getMonths()
				+ "& days to birthday :: " + period.getDays());
